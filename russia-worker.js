...
const CINERAMA_PATH = {
  ...
  // v82.32: НАСТОЯЩИЙ окончательный фикс "Россия 24 без звука" — БЕЗ единого
  // лишнего запроса к Worker (в отличие от отменённого v82.31 mux-segment,
  // который и стал причиной скачка запросов к 75% дневного лимита). Этот же
  // ID (1023) был у Россия 24 ДО v82.5 — тогда его убрали из-за self-loop
  // блокировки датацентрового IP, но с v82.25 у ВСЕХ cinerama.uz-каналов уже
  // есть автообход этой блокировки по 3 зеркалам (см. fetchCinerama) —
  // Россия 24 тогда просто не успела на нём оказаться. Проверено вживую:
  // PMT содержит ОБА потока (H.264 видео + AAC аудио) уже смешанными в одном
  // TS — самый обычный медиа-плейлист, никаких alternate-audio трюков не
  // нужно. hls.js: видео и звук декодируются без единой ошибки.
  rossiya24:     '1023/tracks-v1a1/mono.m3u8',
};
...
// сегмента.
// v82.32: MUXED_AV/mux-segment/muxTsSegments ПОЛНОСТЬЮ УБРАНЫ — именно они
// (проксирование КАЖДОГО TS-сегмента через Worker) разогнали аккаунт до 75%
// дневного лимита запросов Cloudflare за один день. Настоящий фикс звука
// Россия 24 — обычный уже смешанный видео+аудио источник, см. CINERAMA_PATH
// (rossiya24: '1023/...') выше, без единого лишнего запроса к Worker.
...
export default {
  async fetch(request) {
    ...
    if (path === '/version') {
      return new Response(JSON.stringify({ version: VERSION, channels: Object.keys(STREAMS).length + Object.keys(CINERAMA_PATH).length, mode: 'cors-only' }), {
        headers: { ...CORS, 'Content-Type': 'application/json' },
      });
    }
